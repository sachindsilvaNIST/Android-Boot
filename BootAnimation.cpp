...
bool BootAnimation::nidec() {
    ...
    for(int i = 1;i <= 30; i++){
        ...
        // Using the same initTexture method to load each frame - SACHIN.R.DSILVA 2025-01-30
        if(initTexture(&spinnerFrames[i-1], mAssets, filePath) != NO_ERROR){
            ALOGE("Failed to load spinner frame : %s",filePath);
        }
    ...
    do {
        ...
        //------- Drawing the load spinner below NIDEC logo -- SACHIN.R.DSILVA 2025-01-30------------
        glEnable(GL_BLEND); 
